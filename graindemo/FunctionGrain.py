...
#moisture
def getOdateGrainMoisture():
    dataFile = '../data/'
    dataName = 'grain_moisture.mat'
    ...
    datax_train = datax_train.reshape(a, datax_train.shape[1])
    datax_test = datax_test.reshape(b, datax_test.shape[1])

    return datax_test,datay_test,datax_train,datay_train
###########################################################################################################

## 向CSV中写入数据
def write_To_Csv_Moisture(write_data):
    df = pd.DataFrame(write_data,
                      columns=['num', 'model_name', 'epochs', 'batch_size', 'RMSEC', 'R_C', 'RMSEP', 'R_P'])  # 列表数据转为数据框
    df.to_csv('dataResultsGrainMoisture.csv', mode='a', index=False, header=False)
    return
#############################################################################################################
